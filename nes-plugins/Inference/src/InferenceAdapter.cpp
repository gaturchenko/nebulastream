...
void InferenceAdapter::initializeModel(Nebuli::Inference::Model& model, uint64_t batch_size)
{
    ...
    inputSize = model.inputSize() * batch_size;
    inputData = std::make_unique<std::byte[]>(inputSize);

    outputSize = model.outputSize() * batch_size;
    outputData = std::make_unique<std::byte[]>(outputSize);
    reducedInferenceOutputData = std::make_unique<std::byte[]>(outputSize);

    if (backend == Nebuli::Inference::ModelBackend::IREE)
    {
        runtimeWrapper = IREERuntimeWrapper();
    ...
    else if (backend == Nebuli::Inference::ModelBackend::OPENVINO)
    {
    ...
    {
        throw NotImplemented("Unsupported model backend");
    }
...
template <class T>
uint64_t InferenceAdapter::addModelInputPartial(T value)
{
    hasZeroCopyInput = false;
    zeroCopyInputData = nullptr;
...
void InferenceAdapter::addModelInputBatchPartial(int index, std::span<std::byte> content, size_t tupleSize)
{
    hasZeroCopyInput = false;
    zeroCopyInputData = nullptr;
...
template <class T>
void InferenceAdapter::inferWithReduction()
{
    if (backend == Nebuli::Inference::ModelBackend::OPENVINO)
    {
        const std::byte* modelInputData = inputData.get();
        size_t modelInputSize = inputSize;
        switch (currentReductionLevel)
        {
            case LOW:
                lowReductions += 1;
                modelInputData = inputDataHalf.get();
                modelInputSize = std::ceil(1 / float(LOW) * inputSize);
                break;
            case MEDIUM:
                mediumReductions += 1;
                modelInputData = inputDataFourth.get();
                modelInputSize = std::ceil(1 / float(MEDIUM) * inputSize);
                break;
            case HIGH:
                highReductions += 1;
                modelInputData = inputDataEighth.get();
                modelInputSize = std::ceil(1 / float(HIGH) * inputSize);
                break;
            default:
                noReductions += 1;
                break;
        }
        if (modelInputData == nullptr)
        {
            modelInputData = inputData.get();
...
template <class T>
size_t InferenceAdapter::inferCombine(size_t outputSize, size_t outputFields, bool isVarSizedOutput)
{
    if (backend == Nebuli::Inference::ModelBackend::OPENVINO)
    {
        const std::byte* modelInputData = inputData.get();
        ...
        for (const auto outputRowIndex : batchCachingHelper.getMissIndices())
        {
            ...
            {
                std::memcpy(
                    outputData.get() + outputRowIndex * sizeof(T),
                    reducedInferenceOutputData.get() + reducedOutputTupleIndex * outputSize,
                    outputSize);
            }
            else
            {
                std::memcpy(
                    outputData.get() + outputRowIndex * outputSize,
                    reducedInferenceOutputData.get() + reducedOutputTupleIndex * outputSize,
                    outputSize);
            }
            ++reducedOutputTupleIndex;
        }
...
template <class T>
void InferenceAdapter::addModelInput(size_t index, T value)
{
    hasZeroCopyInput = false;
    zeroCopyInputData = nullptr;
...
void InferenceAdapter::addModelInput(std::span<std::byte> content)
{
...
void InferenceAdapter::addModelInputBatch(int index, std::span<std::byte> content, size_t tupleSize)
{
    hasZeroCopyInput = false;
    zeroCopyInputData = nullptr;
...
std::byte* InferenceAdapter::getCurrentInputData()
{
...
template <class T>
T InferenceAdapter::getResultAt(size_t idx)
{
    PRECONDITION(idx < outputSize / sizeof(T), "Index is too large");
    return std::bit_cast<T*>(outputData.get())[idx];
}

void InferenceAdapter::copyResultTo(std::span<std::byte> content)
{
...
void InferenceAdapter::copyResultToBatch(size_t index, std::span<std::byte> content)
{
    std::ranges::copy_n(outputData.get() + index * content.size(), content.size(), content.data());
}

void InferenceAdapter::allocateBuffers(size_t tupleSize)
{
...
std::shared_ptr<InferenceAdapter> InferenceAdapter::create()
{
    auto adapter = std::make_shared<InferenceAdapter>();
    return adapter;
}

#define NES_IREE_ADAPTER_INSTANTIATE(T)                                           \
    template void InferenceAdapter::addModelInput<T>(size_t, T);                        \
    template uint64_t InferenceAdapter::addModelInputPartial<T>(T);                         \
    template T InferenceAdapter::getResultAt<T>(size_t);                                \
    template void InferenceAdapter::infer<T>();                                         \
    template void InferenceAdapter::inferWithReduction<T>();                                         \
    template size_t InferenceAdapter::inferCombine<T>(size_t, size_t, bool);
