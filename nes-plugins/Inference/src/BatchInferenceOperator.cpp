...
BatchInferenceOperator::BatchInferenceOperator(
...
template <typename T>
void BatchInferenceOperator::performInference(
    ...
    const nautilus::val<int*>& deduplicatedOutputRowIndices) const
{
    ...
    for (auto it = pagedVectorRef.begin(fields); it != pagedVectorRef.end(fields); ++it)
    {
        ...
        if (useBatchDeduplication)
        {
            ...
            if (entryRowIndex == rowIndex)
            {
                if (!this->isVarSizedInput)
                {
                    ...
                    for (nautilus::static_val<size_t> i = 0; i < inputs.size(); ++i)
                    {
                        auto index = nautilus::invoke(
                            BatchInference::addUniqueValueToModelProxy<T>,
                ...
                else
                {
                    ...
                    nautilus::invoke(
                        BatchInference::copyUniqueVarSizedToModelProxy,
                        rowIndex,
                        varSizedValue.getContent(),
                        BatchInference::min(varSizedValue.getContentSize(), nautilus::val<uint32_t>(static_cast<uint32_t>(this->inputSize))),
            ...
            nautilus::invoke(BatchInference::setOutputRowIndexProxy, deduplicatedOutputRowIndices, tupleIndex, outputRowIndex);
        }
        ...
        else
        {
            if (!this->isVarSizedInput)
            {
                ...
                {
                    nautilus::invoke(
                        BatchInference::addValueToModelProxy<T>,
                        rowIndex,
                        inputs.at(i).execute(record, executionCtx.pipelineMemoryProvider.arena).cast<nautilus::val<T>>(),
                        operatorHandler,
                        executionCtx.workerThreadId);
            ...
            else
            {
                ...
                const auto varSizedValue = inputValue.cast<VariableSizedData>();
                nautilus::invoke(
                    BatchInference::copyVarSizedToModelProxy,
                    rowIndex,
                    varSizedValue.getContent(),
                    BatchInference::min(varSizedValue.getContentSize(), nautilus::val<uint32_t>(static_cast<uint32_t>(this->inputSize))),
                    nautilus::val<size_t>(inputSize),
    ...
    nautilus::invoke(BatchInference::applyModelProxy<T>, operatorHandler, executionCtx.workerThreadId);
}

template <typename T>
void BatchInferenceOperator::writeOutputRecord(
    const PagedVectorRef& pagedVectorRef,
    TupleBufferRef& tupleBufferRef,
    ExecutionContext& executionCtx,
    const nautilus::val<int*>& deduplicatedOutputRowIndices) const
    ...
    for (auto it = pagedVectorRef.begin(fields); it != pagedVectorRef.end(fields); ++it)
    {
        ...
        if (useBatchDeduplication)
        {
            const auto deduplicatedOutputRowIndex = nautilus::invoke(
                BatchInference::getOutputRowIndexProxy, deduplicatedOutputRowIndices, tupleIndex);
            ...
            if (!this->isVarSizedOutput)
            {
                for (nautilus::static_val<size_t> i = 0; i < outputFieldNames.size(); ++i)
                {
                    VarVal result = VarVal(nautilus::invoke(
                        BatchInference::getValueFromModelProxy<T>,
            ...
            else
            {
                ...
                nautilus::invoke(
                    BatchInference::copyVarSizedFromModelProxy,
...
void BatchInferenceOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    if (useBatchDeduplication)
    {
        deduplicatedOutputRowIndices = nautilus::invoke(
            BatchInference::allocateOutputRowIndicesProxy, batchPagedVectorRef.getNumberOfTuples());
    }
    ...
    if (useBatchDeduplication)
    {
        nautilus::invoke(BatchInference::freeOutputRowIndicesProxy, deduplicatedOutputRowIndices);
    }
...
void BatchInferenceOperator::close(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    const auto operatorHandlerMemRef = executionCtx.getGlobalOperatorHandler(operatorHandlerId);
    nautilus::invoke(BatchInference::garbageCollectBatchesProxy, operatorHandlerMemRef, executionCtx.workerThreadId);
    PhysicalOperatorConcept::close(executionCtx, recordBuffer);
}

void BatchInferenceOperator::setup(ExecutionContext& executionCtx, CompilationContext&) const
{
...
Record
BatchInferenceOperator::createRecord(const Record& featureRecord, const std::vector<Record::RecordFieldIdentifier>& projections) const
{
    Record record;
