...
BatchInferenceOperatorHandler::BatchInferenceOperatorHandler(
...
void BatchInferenceOperatorHandler::stop(QueryTerminationType, PipelineExecutionContext& pipelineExecutionContext)
{
    if (model.getInputs()[0].isType(DataType::Type::VARSIZED))
    {
        uint64_t misses{0};
        uint64_t noReductions{0};
        ...
        for (const auto& adapter : threadLocalAdapters)
        {
            ...
            noReductions += adapter->noReductions;
...
void BatchInferenceOperatorHandler::allocateBuffers(size_t tupleSize)
{
    for (size_t threadId = 0; threadId < numberOfWorkerThreads; ++threadId)
    {
...
void BatchInferenceOperatorHandler::allocateHashMaps(uint64_t keySize, uint64_t valueSize, uint64_t numberOfBuckets, uint64_t pageSize)
{
    for (size_t threadId = 0; threadId < numberOfWorkerThreads; ++threadId)
    {
...
const Nebuli::Inference::Model& BatchInferenceOperatorHandler::getModel() const
{
    return model;
}

const std::shared_ptr<InferenceAdapter>& BatchInferenceOperatorHandler::getAdapter(WorkerThreadId workerThreadId) const
{
    return threadLocalAdapters[workerThreadId % threadLocalAdapters.size()];
}

HashMap* BatchInferenceOperatorHandler::getHashMapPtr(WorkerThreadId workerThreadId) const
{
    return threadLocalHashMaps[workerThreadId % threadLocalHashMaps.size()].get();
}

void BatchInferenceOperatorHandler::clearHashMap(WorkerThreadId workerThreadId)
{
...
std::shared_ptr<Batch> BatchInferenceOperatorHandler::createNewBatch() const
{
    tuplesSeen = 0;
    ++batchId;
...
std::shared_ptr<Batch> BatchInferenceOperatorHandler::getBatch(uint64_t batchId) const
{
...
Batch* BatchInferenceOperatorHandler::getOrCreateNewBatch() const
{
...
void BatchInferenceOperatorHandler::garbageCollectBatches() const
{
    auto batchesWriteLock = batches.wlock();
...
void BatchInferenceOperatorHandler::allocatePredictionCacheEntries(
...
const int8_t* BatchInferenceOperatorHandler::getStartOfPredictionCacheEntries(const StartPredictionCacheEntriesArgs& startPredictionCacheEntriesArgs) const
{
...
uint64_t BatchInferenceOperatorHandler::getReplacementPos(const StartPredictionCacheEntriesArgs& startPredictionCacheEntriesArgs) const
{
    PRECONDITION(!threadLocalAdapters.empty(), "Number of worker threads should be set before calling this method");
    const auto startPredictionCacheEntries = dynamic_cast<const StartPredictionCacheEntriesInference&>(startPredictionCacheEntriesArgs);
...
void
BatchInferenceOperatorHandler::setReplacementPos(const StartPredictionCacheEntriesArgs& startPredictionCacheEntriesArgs, uint64_t idx)
{
    PRECONDITION(!threadLocalAdapters.empty(), "Number of worker threads should be set before calling this method");
    const auto startPredictionCacheEntries = dynamic_cast<const StartPredictionCacheEntriesInference&>(startPredictionCacheEntriesArgs);
    const auto pos = startPredictionCacheEntries.workerThreadId % predictionCacheReplacementPosForWorkerThreads.size();
...
BatchInferenceOperatorHandler::getCreateNewSlicesFunction(const CreateNewSlicesArguments&) const
{
