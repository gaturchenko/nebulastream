...
namespace NES::BatchCacheInference
{
inline BatchInferenceOperatorHandler* getHandler(OperatorHandler* inferModelHandler)
{
    return dynamic_cast<BatchInferenceOperatorHandler*>(inferModelHandler);
}
...
BatchCacheInferenceOperator::BatchCacheInferenceOperator(
...
template <class T>
nautilus::val<std::byte*> BatchCacheInferenceOperator::createCacheProbeTuple(
    ...
    Record& record) const
{
    for (nautilus::static_val<size_t> i = 0; i < inputs.size(); ++i)
    {
        cacheProbeTuple = nautilus::invoke(
            ...
            {
                auto* adapter = BatchCacheInference::getAdapter(inferModelHandler, thread);
...
nautilus::val<std::byte*> BatchCacheInferenceOperator::createCacheProbeTupleVarsized(
    nautilus::val<std::byte*> cacheProbeTuple,
    const nautilus::val<OperatorHandler*>& operatorHandler,
    ExecutionContext& executionCtx,
...
std::pair<nautilus::val<uint64_t>, nautilus::val<std::byte*>> BatchCacheInferenceOperator::probeIntoCache(
...
template <class T>
void BatchCacheInferenceOperator::writeToInputOrOutputBuffer(
    ...
    const nautilus::val<uint64_t>& replacementIndex) const
{
    ...
    if (keyNotFound || !hasCachedPrediction)
    {
        ...
        {
            nautilus::invoke(
                BatchCacheInference::addValueToModelProxy<T>,
                outputRowIndex,
                inputs.at(i).execute(record, executionCtx.pipelineMemoryProvider.arena).cast<nautilus::val<T>>(),
                operatorHandler,
                executionCtx.workerThreadId,
...
void BatchCacheInferenceOperator::writeToInputOrOutputBufferVarsized(
    nautilus::val<std::byte*> prediction,
    const nautilus::val<OperatorHandler*>& operatorHandler,
    ExecutionContext& executionCtx,
    const nautilus::val<int8_t*>& varSizedContent,
    ...
    const nautilus::val<int>& rowIndex) const
{
    ...
    {
        nautilus::invoke(
            BatchCacheInference::copyVarSizedToModelProxy,
            rowIndex,
            varSizedContent,
            varSizedSize,
...
template <class T>
void BatchCacheInferenceOperator::updateCacheValues(
    PredictionCache* predictionCache,
...
void BatchCacheInferenceOperator::updateCacheValuesVarsized(
    PredictionCache* predictionCache,
    const nautilus::val<uint64_t>& cachePos,
    const nautilus::val<OperatorHandler*>& operatorHandler,
    const nautilus::val<WorkerThreadId>& threadId,
    ...
    predictionCache->updateValues(
            ...
            const nautilus::val<PredictionCacheEntry*>& predictionCacheEntryToReplace, const nautilus::val<uint64_t>&)
        {
            return nautilus::invoke(
                ...
                {
                    auto* adapter = BatchCacheInference::getAdapter(opHandlerPtr, thread);
...
template <typename T>
void BatchCacheInferenceOperator::performInference(
    ...
    const nautilus::val<int*>& deduplicatedOutputRowIndices) const
{
    ...
    for (auto it = pagedVectorRef.begin(fields); it != pagedVectorRef.end(fields); ++it)
    {
        ...
        if (useBatchDeduplication)
        {
            ...
            nautilus::val<int> deduplicatedOutputRowIndex = valueRecord.read("rowOutputIndex").cast<nautilus::val<int>>();
            ...
            {
                nautilus::invoke(
                    BatchCacheInference::setOutputRowIndexProxy,
                    deduplicatedOutputRowIndices,
                    tupleIndex,
            ...
            if (!this->isVarSizedInput)
            {
                ...
                deduplicatedOutputRowIndex = outputRowIndex;
                valueRecord.write("rowOutputIndex", VarVal(deduplicatedOutputRowIndex));
            ...
            {
                /// 1. fill a byte array for a record to probe into the cache
                deduplicatedOutputRowIndex = rowIndex;
                valueRecord.write("rowOutputIndex", VarVal(deduplicatedOutputRowIndex));
                entryRef.copyValuesToEntry(valueRecord, executionCtx.pipelineMemoryProvider.bufferProvider);
                ...
                cacheProbeTuple = createCacheProbeTupleVarsized(cacheProbeTuple, operatorHandler, executionCtx, varSizedValue.getContent(),
                    BatchCacheInference::min(varSizedValue.getContentSize(), nautilus::val<uint32_t>(static_cast<uint32_t>(inputSize))));
    ...
    const auto valuesToUpdate = nautilus::invoke(
        BatchCacheInference::applyModelProxy<T>,
        operatorHandler,
        executionCtx.workerThreadId,
        nautilus::val<size_t>(this->outputSize),
...
template <typename T>
void BatchCacheInferenceOperator::writeOutputRecord(
    const PagedVectorRef& pagedVectorRef,
    TupleBufferRef& tupleBufferRef,
    ExecutionContext& executionCtx,
    const nautilus::val<int*>& deduplicatedOutputRowIndices) const
    ...
    for (auto it = pagedVectorRef.begin(fields); it != pagedVectorRef.end(fields); ++it)
    {
        ...
        if (useBatchDeduplication)
        {
            const auto deduplicatedOutputRowIndex = nautilus::invoke(
                BatchCacheInference::getOutputRowIndexProxy, deduplicatedOutputRowIndices, tupleIndex);
            ...
            if (!this->isVarSizedOutput)
            {
                for (nautilus::static_val<size_t> i = 0; i < outputFieldNames.size(); ++i)
                {
                    VarVal result = VarVal(nautilus::invoke(
                        BatchCacheInference::getValueFromModelProxy<T>,
            ...
            else
            {
                ...
                nautilus::invoke(
                    BatchCacheInference::copyVarSizedFromModelProxy,
                    deduplicatedOutputRowIndex,
                    output.getContent(),
...
void BatchCacheInferenceOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    const auto batchRef = nautilus::invoke(
        +[](OperatorHandler* ptrOpHandler, const EmittedBatch* currentBatch)
        {
            ...
            const auto* opHandler = dynamic_cast<BatchInferenceOperatorHandler*>(ptrOpHandler);
    ...
    const auto startOfEntries = nautilus::invoke(
        +[](const BatchInferenceOperatorHandler* opHandler, const WorkerThreadId workerThreadId)
        {
            return opHandler->getStartOfPredictionCacheEntries(
                BatchInferenceOperatorHandler::StartPredictionCacheEntriesInference{workerThreadId});
    ...
    const auto inputTupleSize = nautilus::invoke(
        ...
        {
            auto handler = dynamic_cast<BatchInferenceOperatorHandler*>(inferModelHandler);
            auto adapter = handler->getAdapter(thread);
    ...
    const auto hashMapPtr = nautilus::invoke(
        ...
        {
            return dynamic_cast<BatchInferenceOperatorHandler*>(handler)->getHashMapPtr(threadId);
        }, operatorHandlerRef, executionCtx.workerThreadId);
    ...
    if (useBatchDeduplication)
    {
        deduplicatedOutputRowIndices = nautilus::invoke(
            BatchCacheInference::allocateOutputRowIndicesProxy, batchPagedVectorRef.getNumberOfTuples());
    }
    ...
    if (useBatchDeduplication)
    {
        nautilus::invoke(BatchCacheInference::freeOutputRowIndicesProxy, deduplicatedOutputRowIndices);
    }
    ...
    nautilus::invoke(
        +[](OperatorHandler* ptrOpHandler, WorkerThreadId thread, const EmittedBatch* currentBatch)
        {
            PRECONDITION(ptrOpHandler != nullptr, "opHandler context should not be null!");
            const auto* opHandler = dynamic_cast<BatchInferenceOperatorHandler*>(ptrOpHandler);
            auto adapter = opHandler->getAdapter(thread);
...
void BatchCacheInferenceOperator::setup(ExecutionContext& executionCtx, CompilationContext&) const
{
    ...
    nautilus::invoke(
        ...
        {
            auto handler = dynamic_cast<BatchInferenceOperatorHandler*>(opHandler);
            handler->start(*pec, 0);
    ...
    nautilus::invoke(
        +[](BatchInferenceOperatorHandler* opHandler,
...
void BatchCacheInferenceOperator::close(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    auto* predictionCache = dynamic_cast<PredictionCache*>(executionCtx.getLocalState(id));
    auto operatorHandlerRef = predictionCache->getOperatorHandler();

    nautilus::invoke(
        +[](BatchInferenceOperatorHandler* opHandler, uint64_t pos, const WorkerThreadId workerThreadId)
        {
            opHandler->setReplacementPos(BatchInferenceOperatorHandler::StartPredictionCacheEntriesInference{workerThreadId}, pos);
        }, operatorHandlerRef, predictionCache->getReplacementPos(), executionCtx.workerThreadId);

    nautilus::invoke(BatchCacheInference::garbageCollectBatchesProxy, operatorHandlerRef, executionCtx.workerThreadId);
    PhysicalOperatorConcept::close(executionCtx, recordBuffer);
}

Record
BatchCacheInferenceOperator::createRecord(const Record& featureRecord, const std::vector<Record::RecordFieldIdentifier>& projections) const
{
    Record record;
...
void BatchCacheInferenceOperator::terminate(ExecutionContext& executionCtx) const
{
    nautilus::invoke(
