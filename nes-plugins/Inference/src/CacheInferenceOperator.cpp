...
template <class T>
T getValueFromModelProxy(int index, OperatorHandler* inferModelHandler, WorkerThreadId thread, std::byte* outputData)
{
...
void copyVarSizedFromModelProxy(
    std::byte* content,
    uint32_t size,
...
CacheInferenceOperator::CacheInferenceOperator(
...
template <class T>
nautilus::val<std::byte*> CacheInferenceOperator::performInference(
    ExecutionContext& executionCtx, NES::Record& record, PredictionCache* predictionCache) const
{
    ...
    if (!this->isVarSizedInput)
    {
        ...
        {
            nautilus::invoke(
                CacheInference::addValueToModelProxy<T>,
                nautilus::val<int>(i),
    ...
    else
    {
        ...
        nautilus::invoke(
            CacheInference::copyVarSizedToModelProxy,
            varSizedValue.getContent(),
            CacheInference::min(varSizedValue.getContentSize(), nautilus::val<uint32_t>(static_cast<uint32_t>(this->inputSize))),
            operatorHandler,
            executionCtx.workerThreadId);
    }

    const auto inputDataVal = nautilus::invoke(
        +[](OperatorHandler* inferModelHandler, WorkerThreadId thread)
        {
            auto handler = dynamic_cast<InferenceOperatorHandler*>(inferModelHandler);
    ...
    return predictionCache->getDataStructureRef(
            ...
            const nautilus::val<PredictionCacheEntry*>& predictionCacheEntryToReplace, const nautilus::val<uint64_t>&)
        {
            return nautilus::invoke(
                +[](PredictionCacheEntry* predictionCacheEntry, OperatorHandler* opHandlerPtr, WorkerThreadId thread)
                {
                    auto handler = dynamic_cast<InferenceOperatorHandler*>(opHandlerPtr);
                    auto adapter = handler->getAdapter(thread);
                    adapter->infer<T>();
                    adapter->misses += 1;
...
template <class T>
void CacheInferenceOperator::writeOutputRecord(
    ...
    PredictionCache* predictionCache) const
{
    ...
    if (!this->isVarSizedOutput)
    {
        for (nautilus::static_val<size_t> i = 0; i < outputFieldNames.size(); ++i)
        {
            const VarVal result = VarVal(nautilus::invoke(
                CacheInference::getValueFromModelProxy<T>,
                nautilus::val<int>(i),
                operatorHandler,
                executionCtx.workerThreadId,
                prediction));
    ...
    else
    {
        ...
        nautilus::invoke(
            CacheInference::copyVarSizedFromModelProxy,
            output.getContent(),
...
void CacheInferenceOperator::execute(ExecutionContext& executionCtx, NES::Record& record) const
{
...
void CacheInferenceOperator::setup(ExecutionContext& executionCtx, CompilationContext&) const
{
    ...
    nautilus::invoke(
        +[](InferenceOperatorHandler* opHandler,
...
void CacheInferenceOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    nautilus::invoke(
        +[](uint64_t n)
        {
    ...
    const auto startOfEntries = nautilus::invoke(
        +[](const InferenceOperatorHandler* opHandler, const WorkerThreadId workerThreadId)
        {
            return opHandler->getStartOfPredictionCacheEntries(
                InferenceOperatorHandler::StartPredictionCacheEntriesInference{workerThreadId});
...
void CacheInferenceOperator::close(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    auto* predictionCache = dynamic_cast<PredictionCache*>(executionCtx.getLocalState(id));
    auto inferModelHandler = predictionCache->getOperatorHandler();

    nautilus::invoke(
        +[](InferenceOperatorHandler* opHandler, uint64_t pos, const WorkerThreadId workerThreadId)
        {
            opHandler->setReplacementPos(InferenceOperatorHandler::StartPredictionCacheEntriesInference{workerThreadId}, pos);
...
void CacheInferenceOperator::terminate(ExecutionContext& executionCtx) const
{
    nautilus::invoke(
