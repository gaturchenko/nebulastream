    ...
    ExecutionContext& executionCtx) const
{
    ...
    const auto pred = predictionCache->getDataStructureRef(
        inputDataVal,
        [&](
            const nautilus::val<PredictionCacheBatchEntry*>& predictionCacheEntryToReplace, const nautilus::val<uint64_t>&)
        {
            return nautilus::invoke(
                +[](PredictionCacheBatchEntry* predictionCacheEntry, void* opHandlerPtr, WorkerThreadId thread)
                {
...
void IREEBatchCacheInferenceOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    predictionCache->updateEntryData(startOfEntries, inputSize);
...
void IREEBatchCacheInferenceOperator::setup(ExecutionContext& executionCtx) const
{
    ...
    nautilus::invoke(
        ...
        predictionCache->getSizeOfEntry(),
        predictionCache->getNumberOfEntries());
}
