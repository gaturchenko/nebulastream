...
namespace NES
{
PredictionCacheBatchLRU::PredictionCacheBatchLRU(
...
nautilus::val<uint64_t*> PredictionCacheBatchLRU::getAgeBit(const nautilus::val<uint64_t>& pos)
{
    const auto PredictionCacheBatchEntry = startOfEntries + pos * sizeOfEntry;
    const auto ageBitRef = Nautilus::Util::getMemberRef(PredictionCacheBatchEntry, &PredictionCacheBatchEntryLRU::ageBit);
    return ageBitRef;
}

nautilus::val<int8_t*>
PredictionCacheBatchLRU::getDataStructureRef(const nautilus::val<std::byte*>& record, const PredictionCacheBatch::PredictionCacheReplacement& replacementFunction)
{
    ...
    const nautilus::val<PredictionCacheBatchEntry*> PredictionCacheBatchEntryToReplace = startOfEntries + maxAgeIndex * sizeOfEntry;
    const auto dataStructure = replacementFunction(PredictionCacheBatchEntryToReplace, maxAgeIndex);
