...
#include <PredictionCacheBatch/PredictionCacheBatch2Q.hpp>
...
namespace NES
{
PredictionCacheBatch2Q::PredictionCacheBatch2Q(
...
void PredictionCacheBatch2Q::movePredictionCacheBatchEntryToLRUQueue(const nautilus::val<uint64_t>& fifoPos, const nautilus::val<uint64_t>& lruPos)
{
...
nautilus::val<int8_t*>
PredictionCacheBatch2Q::getDataStructureRef(const nautilus::val<std::byte*>& record, const PredictionCacheBatch::PredictionCacheReplacement& replacementFunction)
{
    ...
    for (nautilus::val<uint64_t> i = lruQueueSize; i < lruQueueSize + fifoQueueSize; i = i + 1)
    {
        ...
        {
            incrementNumberOfHits();
            movePredictionCacheBatchEntryToLRUQueue(i, maxAgeIndex);
            return getDataStructure(i);
        }
    }
    ...
    const nautilus::val<PredictionCacheBatchEntry*> PredictionCacheBatchEntryToReplace = startOfFifoEntries + fifoReplacementOffset;
    const auto dataStructure = replacementFunction(PredictionCacheBatchEntryToReplace, lruQueueSize + fifoReplacementIndex);
...
nautilus::val<uint64_t*> PredictionCacheBatch2Q::getAgeBit(const nautilus::val<uint64_t>& pos)
{
    const auto PredictionCacheBatchEntry = startOfLRUEntries + pos * sizeOfEntry;
    const auto ageBitRef = Nautilus::Util::getMemberRef(PredictionCacheBatchEntry, &PredictionCacheBatchEntry2Q::ageBit);
    return ageBitRef;
}
}
