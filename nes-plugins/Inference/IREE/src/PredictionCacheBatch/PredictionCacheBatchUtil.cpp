...
#include <PredictionCacheBatch/PredictionCacheBatchFIFO.hpp>
#include <PredictionCacheBatch/PredictionCacheBatchLFU.hpp>
#include <PredictionCacheBatch/PredictionCacheBatchLRU.hpp>
...
namespace NES::Util
{
std::shared_ptr<PredictionCacheBatch> createPredictionCacheBatch(
    ...
    const nautilus::val<size_t>& inputSize)
{
    ...
    const auto predictionCacheEntries = startOfEntries + nautilus::val<uint64_t>(sizeof(BatchHitsAndMisses));
    const auto startOfDataEntry = nautilus::invoke(
        +[](const PredictionCacheBatchEntry* predictionCacheEntry)
        {
    ...
    switch (predictionCacheOptions.predictionCacheType)
    {
        ...
        case NES::Configurations::PredictionCacheType::FIFO:
            return std::make_shared<PredictionCacheBatchFIFO>(
                predictionCacheOptions.numberOfEntries,
                sizeof(PredictionCacheBatchEntryFIFO),
                predictionCacheEntries,
                hitsRef,
                missesRef,
                inputSize);
        case NES::Configurations::PredictionCacheType::LFU:
            return std::make_shared<PredictionCacheBatchLFU>(
                predictionCacheOptions.numberOfEntries,
                sizeof(PredictionCacheBatchEntryLFU),
                predictionCacheEntries,
                hitsRef,
                missesRef,
                inputSize);
        case NES::Configurations::PredictionCacheType::LRU:
            return std::make_shared<PredictionCacheBatchLRU>(
                predictionCacheOptions.numberOfEntries,
                sizeof(PredictionCacheBatchEntryLRU),
                predictionCacheEntries,
                hitsRef,
                missesRef,
                inputSize);
        case NES::Configurations::PredictionCacheType::SECOND_CHANCE:
            return std::make_shared<PredictionCacheBatchSecondChance>(
                predictionCacheOptions.numberOfEntries,
                sizeof(PredictionCacheBatchEntrySecondChance),
                predictionCacheEntries,
                hitsRef,
                missesRef,
                inputSize);
        case NES::Configurations::PredictionCacheType::TWO_QUEUES: {
            ...
            const nautilus::val<uint64_t> fifoQueueStartOffset = lruQueueSize * sizeof(PredictionCacheBatchEntry2Q);
            const auto startOfFifoEntries = startOfLRUEntries + fifoQueueStartOffset;

            return std::make_shared<PredictionCacheBatch2Q>(
                predictionCacheOptions.numberOfEntries,
                sizeof(PredictionCacheBatchEntry2Q),
                predictionCacheEntries,
                hitsRef,
                missesRef,
                startOfFifoEntries,
