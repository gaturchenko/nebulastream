...
namespace NES
{
struct PredictionCacheBatchEntryLRU : PredictionCacheBatchEntry
{
    /// Stores the age of each entry in the cache. With 32-bits, we can store 4 billion entries.
    uint64_t ageBit;
    ~PredictionCacheBatchEntryLRU() override = default;
};

class PredictionCacheBatchLRU final : public PredictionCacheBatch
{
public:
    PredictionCacheBatchLRU(
    ...
    ~PredictionCacheBatchLRU() override = default;
    nautilus::val<int8_t*>
