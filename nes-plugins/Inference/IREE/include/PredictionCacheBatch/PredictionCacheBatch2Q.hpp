...
namespace NES
{
struct PredictionCacheBatchEntry2Q : PredictionCacheBatchEntry
{
    /// Stores the age of each entry in the cache. With 32-bits, we can store 4 billion entries.
    uint64_t ageBit;
    ~PredictionCacheBatchEntry2Q() override = default;
};
...
class PredictionCacheBatch2Q final : public PredictionCacheBatch
{
public:
    PredictionCacheBatch2Q(
    ...
    ~PredictionCacheBatch2Q() override = default;
    nautilus::val<int8_t*>
...
private:
    ...
    void movePredictionCacheBatchEntryToLRUQueue(const nautilus::val<uint64_t>& fifoPos, const nautilus::val<uint64_t>& lruPos);
