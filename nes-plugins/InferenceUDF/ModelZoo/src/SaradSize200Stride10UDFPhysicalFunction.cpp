...
namespace
{
constexpr auto FunctionName = "SARAD_SIZE_200_STRIDE_10_UDF";
constexpr auto ModelEnv = "NES_SARAD_SIZE_200_STRIDE_10_UDF_MODEL";
constexpr auto ModelPath = "nes-systests/testdata/model/cwru_bearing/pretrained/sarad/sarad-size-200-stride-10.onnx";
...
using SaradPhysicalFunction = ModelZooUDFPhysicalFunction<outputSize, infer>;
static_assert(PhysicalFunctionConcept<SaradPhysicalFunction>);
}

PhysicalFunctionRegistryReturnType
PhysicalFunctionGeneratedRegistrar::RegisterSARAD_SIZE_200_STRIDE_10_UDFPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1, "{} function must have exactly one child function", FunctionName);
    return SaradPhysicalFunction(arguments.childFunctions[0]);
}
