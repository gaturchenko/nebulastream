...
namespace
{
constexpr auto FunctionName = "MHATT_RNN_UDF";
constexpr auto ModelEnv = "NES_MHATT_RNN_UDF_MODEL";
constexpr auto ModelPath = "nes-systests/testdata/model/musan/pretrained/mhatt-rnn/saved_model";
...
using MhattRnnPhysicalFunction = ModelZooUDFPhysicalFunction<outputSize, infer>;
static_assert(PhysicalFunctionConcept<MhattRnnPhysicalFunction>);
}

PhysicalFunctionRegistryReturnType
PhysicalFunctionGeneratedRegistrar::RegisterMHATT_RNN_UDFPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1, "{} function must have exactly one child function", FunctionName);
    return MhattRnnPhysicalFunction(arguments.childFunctions[0]);
}
