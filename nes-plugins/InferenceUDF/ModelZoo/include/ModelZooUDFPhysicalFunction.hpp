...
#ifndef NES_MODEL_ZOO_UDF_SOURCE_DIR
#define NES_MODEL_ZOO_UDF_SOURCE_DIR ""
#endif

#ifndef NES_MODEL_ZOO_UDF_BINARY_DIR
#define NES_MODEL_ZOO_UDF_BINARY_DIR ""
#endif

namespace NES
{

[[nodiscard]] inline std::filesystem::path modelZooUdfAbsolutePath(const std::filesystem::path& path)
{
...
[[nodiscard]] inline std::filesystem::path modelZooUdfModelPath(
    const char* environmentVariable, const std::filesystem::path& modelRelativePath)
{
    ...
    {
        return modelZooUdfAbsolutePath(path);
    }

    const auto sourceDir = std::filesystem::path{NES_MODEL_ZOO_UDF_SOURCE_DIR};
    const auto binaryDir = std::filesystem::path{NES_MODEL_ZOO_UDF_BINARY_DIR};
    const std::array<std::filesystem::path, 7> candidates{
        binaryDir / modelRelativePath,
        sourceDir / modelRelativePath,
        sourceDir / "cmake-build-debug" / modelRelativePath,
        sourceDir / "cmake-build-release" / modelRelativePath,
        sourceDir / "cmake-build-relwithdebinfo" / modelRelativePath,
        modelRelativePath,
        std::filesystem::path{"model"} / modelRelativePath.lexically_relative("nes-systests/testdata/model")};
    for (const auto& candidate : candidates)
    {
        if (!candidate.empty() && std::filesystem::exists(candidate))
        {
            return modelZooUdfAbsolutePath(candidate);
        }
    }
    return modelZooUdfAbsolutePath(binaryDir / modelRelativePath);
}
...
class ModelZooUDFPhysicalFunction final
{
public:
    explicit ModelZooUDFPhysicalFunction(PhysicalFunction childPhysicalFunction)
