...
#include "../include/ToBase64PhysicalFunction.hpp"
...
uint32_t encodeBase64(int8_t* inputPtr, uint32_t inputSize, int8_t* outputPtr)
{
    ...
    return static_cast<uint32_t>(encodedLen);
}
}
...
VarVal ToBase64PhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto inputSize = inputValue.getContentSize();

    /// Base64 output is at most 4/3 of input size + padding, rounded up to multiple of 4
    const auto maxOutputSize = (inputSize + nautilus::val<uint32_t>(2)) / nautilus::val<uint32_t>(3) * nautilus::val<uint32_t>(4);
    ...
    VarVal(actualSize).writeToMemory(output.getReference());

    return VariableSizedData(output.getReference(), actualSize);
}

PhysicalFunctionRegistryReturnType
PhysicalFunctionGeneratedRegistrar::RegisterToBase64PhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 1, "ToBase64 function must have exactly one child function");
