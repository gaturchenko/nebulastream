...
uint32_t decodeBase64(int8_t* inputPtr, uint32_t inputSize, int8_t* outputPtr)
{
    ...
    auto actualLen = static_cast<uint32_t>(decodedLen);
...
VarVal FromBase64PhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto maxOutputSize = inputSize / nautilus::val<uint32_t>(4) * nautilus::val<uint32_t>(3);
...
PhysicalFunctionGeneratedRegistrar::RegisterFromBase64PhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 1, "FromBase64 function must have exactly one child function");
