...
/// @brief Scan-style physical operator that combines batch inference with a per-worker prediction cache.
class BatchCacheInferModelPhysicalOperator final : public PhysicalOperatorConcept
{
public:
    BatchCacheInferModelPhysicalOperator(
        CompiledModel model,
...
private:
    std::shared_ptr<detail::ThreadLocalBatchPredictionCacheWrapper> threadLocal;
    std::shared_ptr<TupleBufferRef> bufferRef;
    std::vector<Record::RecordFieldIdentifier> projections;
    std::vector<std::string> inputFieldNames;
    std::vector<std::string> outputFieldNames;
